  ...
  return (
    ...
    <Carousel autoplay autoplaySpeed={5000} pauseOnHover={false}>
      {data?.data?.map((item: TProduct) => (
        <div className="w-full h-[360px] overflow-hidden relative rounded-md">
          <img
          ...
          <div className="absolute w-full h-full left-0 top-0 flex flex-col justify-center items-center text-center text-white bg-black/30">
            ...
            <p className="text-base mb-2 px-5">
              {item?.description.slice(0, 137)}...
            </p>
            <button
              onClick={() => navigate(`/products/${item.id}`)}
              className="bg-accentColor px-2 rounded-md py-1 text-white hover:bg-black"
            >
              Order Now
            </button>
          </div>
